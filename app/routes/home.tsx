...
// Zodiac sign data
const zodiacSigns = [
  { sign: "aries", name: "Aries", dates: "Mar 21 - Apr 19", emoji: "♈", element: "Fire", planet: "Mars" },
  { sign: "taurus", name: "Taurus", dates: "Apr 20 - May 20", emoji: "♉", element: "Earth", planet: "Venus" },
  { sign: "gemini", name: "Gemini", dates: "May 21 - Jun 20", emoji: "♊", element: "Air", planet: "Mercury" },
  { sign: "cancer", name: "Cancer", dates: "Jun 21 - Jul 22", emoji: "♋", element: "Water", planet: "Moon" },
  { sign: "leo", name: "Leo", dates: "Jul 23 - Aug 22", emoji: "♌", element: "Fire", planet: "Sun" },
  { sign: "virgo", name: "Virgo", dates: "Aug 23 - Sep 22", emoji: "♍", element: "Earth", planet: "Mercury" },
  { sign: "libra", name: "Libra", dates: "Sep 23 - Oct 22", emoji: "♎", element: "Air", planet: "Venus" },
  { sign: "scorpio", name: "Scorpio", dates: "Oct 23 - Nov 21", emoji: "♏", element: "Water", planet: "Pluto" },
  { sign: "sagittarius", name: "Sagittarius", dates: "Nov 22 - Dec 21", emoji: "♐", element: "Fire", planet: "Jupiter" },
  { sign: "capricorn", name: "Capricorn", dates: "Dec 22 - Jan 19", emoji: "♑", element: "Earth", planet: "Saturn" },
  { sign: "aquarius", name: "Aquarius", dates: "Jan 20 - Feb 18", emoji: "♒", element: "Air", planet: "Uranus" },
  { sign: "pisces", name: "Pisces", dates: "Feb 19 - Mar 20", emoji: "♓", element: "Water", planet: "Neptune" },
];

// Quiz questions for the Cosmic Personality Quiz
const quizQuestions = [
  {
  ...
  },
  {
    ...
    options: [
      ...
      { text: "By asking others and gathering multiple perspectives", value: 2 },
      { text: "I avoid difficult problems and hope they resolve themselves", value: 3 }
    ]
  }
];
...
function MainContent() {
  ...
  const selectAnswer = (answerValue: number) => {
    ...
    if (currentQuestion < quizQuestions.length - 1) {
      ...
      setDeterminedSign(zodiacSigns[signIndex].sign);
      setQuizComplete(true);
      
      // Fetch CAPTCHA for this sign
      fetchCaptcha(zodiacSigns[signIndex].sign);
    }
  };

  // Determine zodiac sign based on quiz answers
  const determineZodiacSign = (quizAnswers: number[]): number => {
  ...
  // Fetch CAPTCHA for the determined sign
  const fetchCaptcha = async (sign: string) => {
    try {
      ...
      setCaptchaError("Failed to generate your astrological challenge. The stars must be misaligned!");
    } finally {
  ...
  const validateCaptcha = async () => {
    try {
      ...
      setCaptchaError("The cosmic forces rejected your answer. Please try again.");
    } finally {
      setCaptchaLoading(false);
    }
  };

  // Fetch full horoscope
  const fetchHoroscope = async (sign: string | null) => {
    if (!sign) return;
    ...
    try {
      ...
      setHoroscopeError("The stars couldn't align to reveal your horoscope. Mercury must be in retrograde!");
    } finally {
  ...
  const formatCaptchaInstruction = (instruction: string, horoscopeText: string) => {
    if (!instruction || !horoscopeText) return instruction;

    // Find words from the horoscope mentioned in the instruction
    const horoscopeWords = horoscopeText.split(/\s+/).filter(word => word.length > 3);
    
    let formattedInstruction = instruction;
    
    // Highlight any words from the horoscope that appear in the instruction
    horoscopeWords.forEach(word => {
      const cleanWord = word.replace(/[.,!?;:'"()]/g, '');
      if (cleanWord.length > 3) {
        const regex = new RegExp(`\\b${cleanWord}\\b`, 'gi');
        formattedInstruction = formattedInstruction.replace(regex, match => 
          `<span class="font-bold text-purple-500 dark:text-purple-300">${match}</span>`
        );
      }
    });
  ...
  const renderStartScreen = () => (
    <div className="text-center py-10">
      ...
      <p className="mb-8 text-gray-700 dark:text-gray-300">
        Discover what the stars have pessimistically prepared for your future through our
        scientifically dubious personality assessment.
      </p>
      <button
        onClick={startQuiz}
  ...
  const renderSignReveal = () => {
    ...
    return (
      <div className="py-6 text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-1">{signData.dates}</p>
        ...
        <div className="p-4 bg-purple-100 dark:bg-purple-900/30 rounded-lg mb-6 max-w-md mx-auto">
          <p className="text-sm italic">
            Through rigorous cosmic analysis, our algorithm has determined you are a{" "}
            <span className="font-bold">{signData.name}</span>!
          </p>
  ...
  const renderCaptcha = () => {
    ...
    if (captchaError) {
      return (
        <div className="py-6 text-center">
          ...
          <button
            onClick={() => fetchCaptcha(determinedSign!)}
    ...
    if (captchaSuccess) {
      return (
        <div className="py-6 text-center">
          <p className="text-green-600 dark:text-green-400 font-bold">{captchaMessage}</p>
          {horoscopeLoading && (
            <p className="mt-4 animate-pulse">Decoding your cosmic destiny...</p>
          )}
        </div>
      );
    }
    ...
    const formattedCaptchaInstruction = formatCaptchaInstruction(
      ...
      <div className="py-6 max-w-xl mx-auto">
        ...
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 mb-6">
          <h4 className="font-bold text-sm mb-2 text-yellow-800 dark:text-yellow-200">
            Your Personal Daily {captchaData.sign.charAt(0).toUpperCase() + captchaData.sign.slice(1)} Horoscope:
          </h4>
        ...
        <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4 mb-4">
          ...
          <p 
            className="text-sm" 
            dangerouslySetInnerHTML={{ __html: formattedCaptchaInstruction }} 
          />
        </div>
        ...
        {captchaMessage && !captchaSuccess && (
          <p className="mt-2 text-red-500 text-sm">{captchaMessage}</p>
        )}
      </div>
    );
  };
          ...
          <div className="flex justify-between items-center mb-4">
            <div>
              <h4 className="text-xl font-bold">
                {horoscope.sign.charAt(0).toUpperCase() + horoscope.sign.slice(1)} Horoscope
              </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
              ...
              <div className="flex items-center gap-2">
                <div className="h-4 flex-1 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-purple-600 dark:bg-purple-400 rounded-full"
                    style={{ width: `${horoscope.cosmicPower * 10}%` }}
                  ></div>
                </div>
                <span className="text-sm font-mono">{horoscope.cosmicPower}/10</span>
              </div>
            </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
              ...
              <ul className="text-sm space-y-1">
                <li><span className="font-bold">Color:</span> {horoscope.luckyColor}</li>
                <li><span className="font-bold">Number:</span> {horoscope.luckyNumber}</li>
                <li><span className="font-bold">Emoji:</span> {horoscope.luckyEmoji}</li>
              </ul>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                {horoscope.compatibleSigns.map((sign: string) => {
                  ...
                  return (
                    <div key={sign} className="bg-green-50 dark:bg-green-900/20 p-2 rounded flex items-center gap-1">
                      <span>{signData?.emoji}</span>
                      <span className="text-sm">{sign.charAt(0).toUpperCase() + sign.slice(1)}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                {horoscope.incompatibleSigns.map((sign: string) => {
                  const signData = zodiacSigns.find(s => s.sign === sign);
                  return (
                    <div key={sign} className="bg-red-50 dark:bg-red-900/20 p-2 rounded flex items-center gap-1">
                      <span>{signData?.emoji}</span>
                      <span className="text-sm">{sign.charAt(0).toUpperCase() + sign.slice(1)}</span>
                    </div>
                  );
